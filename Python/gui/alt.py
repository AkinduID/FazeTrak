...
from PyQt5.QtCore import QThread, pyqtSignal as Signal, pyqtSlot as Slot, QTimer
import sys
import cv2
    ...
    def run(self):
        while self.running and self.cap.isOpened():
            ...
            if face_results.face_landmarks:
                ...
                cv2.line(image_rgb, (int(self.center_x), int(self.center_y)), (int(face_center_x), int(face_center_y)),
                         (0, 255, 0), 2)
            ...
            self.vcam.send(cv2.cvtColor(flip_image, cv2.COLOR_BGR2RGB))
...
class MainApp(QtWidgets.QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Face Tracking Application with Arduino Detection")
    ...
    def check_arduino_connection(self):
        ...
        for port in ports:
            if "Arduino" in port.description:
                self.arduino_status.setText(f"Arduino Status: Connected - {port.device}")
                return
